"""Reference:https://colab.research.google.com/drive/1atRCCsx_an8Y-CXFMy5z_H6J_ErzZALN?usp=sharing"""
""" Make sure everything in requirements.txt are installed"""
...
# If `own_model`= True then use your own model else use the model from hugging face
own_model = False
...
def exec_predictions(question_texts, context_text):
    """Setup function to compute predictions"""
    examples = []

    for i, question_text in enumerate(question_texts):
        example = SquadExample(qas_id=str(i),question_text=question_text,context_text=context_text,answer_text=None,start_position_character=None,title="Predict",is_impossible=False,answers=None,)
        examples.append(example)

    features, dataset = squad_convert_examples_to_features(examples=examples,tokenizer=tokenizer,max_seq_length=384,doc_stride=128,max_query_length=64,is_training=False,return_dataset="pt",threads=1,)
